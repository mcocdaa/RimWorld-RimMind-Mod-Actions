...
**Execute 流程**：查找 IActionRule → 检查 `IsAllowed(intentId)` → 调用 `rule.Execute()`
...
| eat_food | Medium | Y | 吃指定食物 | 食物关键词（可选，大小写不敏感匹配 defName 或 Label） | - | 先搜背包再搜地图（按距离）；自动计算 Ingest stackCount |
...
| cancel_job | Medium | - | 中止当前任务 | - | - | EndCurrentJob(Incompletable)；检查 curJob != null |
...
- AI 动作描述：`RimMind.Actions.Desc.{intentId}`
